...
///
/// `\x1b[?6l` (origin mode off) は **`\x1b7` の前** に出す。`\x1b7` (DECSC) は
/// origin mode も保存し `\x1b8` (DECRC) で復元するため、save/restore の内側で
/// origin mode を変更しても恒久化しない。直前の TUI コマンド (top 等) が origin
/// mode を on にしたまま抜けた場合に、status bar 描画位置が 1 行ズレる事故を
/// 防ぐためにこの順序が重要。
pub fn resize_status_bar(rows: u16) {
    ...
    let scroll_bottom = rows.saturating_sub(1).max(1);
    let _ = write!(stdout,
        "\x1b[?6l\x1b7\x1b[1;{scroll_bottom}r\x1b[{rows};1H{color}{label}\x1b[K\x1b[0m\x1b8"
    );
    let _ = stdout.flush();
}
