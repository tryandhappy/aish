...
- **スコープ外（既知の制約）**: `--update` 自己更新は Windows では実行しない（実行中 exe の rename 不可・sha256sum 不在など独立課題）。**ただし bare error でなく、`run_update` 冒頭の `cfg!(windows)` 分岐で `windows_update_hint(channel)`（README Installation > Windows と同一の PowerShell 再インストール手順。tag 解決行だけ stable=`/releases/latest` / prerelease=`/releases[0]` に追従）を表示して `Ok(())` 終了する**（`detect_target()` の `Unsupported platform` Err より前に分岐。純関数なので `windows_update_hint_is_channel_aware` で内容固定）。リリースへの Windows バイナリ（x86_64/aarch64 の生 exe + zip）は `release.yml` の matrix に同梱済み（2026-07 実機検証完了後に追加）。Ctrl+Break は console ctrl event として aish を即殺し得る（必要なら SetConsoleCtrlHandler で無視、初期は文書化のみ）。legacy conhost / mintty は対象外（`console_ok()` が拒否）。
