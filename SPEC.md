...
- **Antigravity（`agy`）/ Grok（`grok`）も表外**（Gemini 行と同型の system-prompt-only backend、§15.10）。Antigravity=`agy -p`（stdin）/ 実行ファイル `agy` / JSON 強制なし / 危険ツール無効化は system prompt のみ / resume `agy --continue`（best-effort）/ **reasoning effort `--effort low|medium|high` は native 対応**。Grok=`grok --prompt-file /dev/stdin`（Unix、stdin）/ `grok -p <prompt>`（Windows）/ 実行ファイル `grok` / **JSON 強制 `--json-schema`（claude と同じ `AI_RESPONSE_SCHEMA` を共有。封筒 `text` を構造化パース、失敗時 lossy fallback）** / 危険ツール無効化は system prompt のみ / resume なし（内部 history）/ **effort `--reasoning-effort low|medium|high|xhigh` は native 対応** / model は `-m`（一覧は `grok models` 動的取得）。両者とも headless で read-only/plan の permission-layer 強制を持たないため `--dangerously-skip-permissions` / `--always-approve` は絶対に付けない（§15.10）。
...
| `confirm_color` | `\x1b[38;5;153;48;5;239m`（淡青 + 灰背景） |
...
- **危険度分類の metadata は信頼境界の外**（2026-09〜）。`ProposedCommand.risk` は**表示専用**で、`VettedCommand` にも `send_approved_command` の送信バイトにも一切入れない。`confirm_and_execute` は loop index で `&ProposedCommand` を並走参照し、`command` だけを `Cow`→`VettedCommand` に載せる。`e=編集`しても risk は index 固定の元コマンド由来をそのまま維持（編集後のバイトに対する分類ではない点は許容 = ユーザ合意）。確認プロンプトは `build_confirm_prompt`（純関数・golden test）が**コマンド本文だけ**を危険度色（`ui::risk_color`: Green=`38;5;108` / Yellow=`38;5;178` / Orange=`38;5;166` / Red=`38;5;196`、ハードコード）で描画。**2026-09 に配色を見直し**: 従来のネオン原色（緑 40 / 黄 226 / 橙 208）は目立ちすぎ、かつ橙 208 は aish ブランド色（`[aish]` プロンプト・スピナー）と同一で「aish が話す色」と「危険度色」が混同したため、トーンダウンした値（緑は当初 71、その後さらに彩度を落としたセージ 108 / 黄 178 / 橙は深橙 166）へ変更（赤 196 は危険を目立たせるため維持）。`Exec?` ラベルは**明黄 + 灰背景**（`38;5;228;48;5;239`）で描画する（2026-09 ユーザ選択）。当初オレンジ+暗茶背景で prompt_color と同一だったため一旦青系（`38;5;117`+濃紺）にしたが、最終的に明黄+灰背景に決定。aish プロンプト（オレンジ+暗茶背景）とは**背景色**で、確認ブラケット（淡青+灰背景）とは**前景色**で区別する（危険度色 228 は不使用なので被らない）。`[Y/n/e/a/q]` ブラケットは confirm_color を維持（ブラケットは bold+reverse なので危険度色を当てると派手すぎる = ユーザ指摘）。confirm_color 既定も暖色（淡黄 228）から寒色（淡青 `38;5;153` + 灰背景 239）へ変更（`[display]` で上書き可）。`print_ai_commands` の事前一覧も各コマンドを危険度色に。**コマンドごとの説明文は当初持たせたが表示が煩雑になるため撤去**（explanation を schema/prompt/型/表示から削除。2026-09）。

### 15.8 その他の UI ルール

- **起動バナー（`print_startup_banner`）は PTY spawn より前に描画する**。Windows Terminal + PowerShell（ConPTY）では、**ConPTY が spawn 時点の実カーソル位置を基準に子シェルの描画をアンカーする**。spawn 後にバナーを出すと、ConPTY 内で生成された初回プロンプト（row1 への絶対位置 `\x1b[H` 等を含む）が最初の `drain_pty` で流れてバナーを上書きする（2026-07 実測: 起動バナーが一瞬出て PowerShell プロンプトに消える）。バナーを spawn 前に出し、カーソルをバナー下へ進めてから spawn すれば ConPTY はバナーの下にアンカーされる（`main.rs` で `print_startup_banner` を `spawn_local_shell` の直前に配置）。**追補（2026-08 実測）**: spawn 前バナー配置をしても「起動バナーが最初のプロンプトで消える」現象が Windows Terminal + PowerShell 5.1 で残っていた。`AISH_DEBUG_PTY` 相当の実出力採取で原因が判明: バナーは上書きされておらず（スクロールで戻ると残存）、**ConPTY 内で起動した子 `powershell.exe` が自分の起動ロゴ（`Windows PowerShell` / `Copyright ...` / `新機能と改善のために最新の PowerShell をインストールしてください!`）を再出力**し、その 4〜5 行ぶんビューポートが下へスクロールして aish バナーが画面外へ押し出されていた（＝アンカリング/上書きではなくスクロールアウト）。対策として **PowerShell 系子シェル（`powershell`/`pwsh`）を `-NoLogo` 付きで spawn** する（`pty_handler::spawn_local_shell` が `is_powershell_shell` で shell 名を検出し `-NoLogo` を付与。cmd/他 shell には付けない）。`-NoLogo` は起動ロゴ抑止のみでプロンプト形・履歴・コマンド実行を変えず、`PromptSniffer` にも非干渉。多くの環境では更新通知（`aka.ms/PSWindows`）も `-NoLogo` で一緒に消えるが、残る環境向けの `POWERSHELL_UPDATECHECK=Off` 注入は「子 env を触る」ため保留（ユーザ合意 2026-08、まず `-NoLogo` 単独で運用）。**追補 2（2026-08 実測、`portable_pty` プローブで確定）**: `-NoLogo` 後もバナーが消える環境が残った。真因はスクロールアウトでなく **ConPTY が spawn 直後に全画面クリア（`\x1b[?25l\x1b[2J\x1b[m\x1b[H` + 初回プロンプト）を必ず emit する**こと（= spawn 前に何を描いても消される。「アンカリング」モデル自体が誤りだった）。対策として **Windows のローカルシェルではバナーを spawn 前でなく、初期バーストを吸収した後に `main::windows_local_startup` が描画する**（§ 15.14）。Unix / SSH は従来どおり spawn 前描画（Windows + SSH ではバナーが初期クリアで消える既知の劣化モード。リモートへの空 Enter 注入はしない方針のため）。**旧 Bug 2（AI 応答行の上書き）は ConPTY 再同期（`conpty_sync`、§ 15.14）で解決済み（2026-08）**。
- **PTY 出力の実測は `AISH_DEBUG_PTY=1`**: `drain_pty` が受け取った生チャンクを `debug_bytes` で escape して **stderr** に出す（`main::debug_pty`。`AISH_DEBUG_KEYS` と対で既定無効・OnceLock キャッシュ）。**aish が PTY へ書いたバイト列も `[aish-pty-w ...]` で対にダンプする**（`main::debug_pty_write`、`PtyHandler::write` 経由の全書き込み。読み書きを突き合わせて「送ったキーが子にどう届いたか」を実測する用途。win32-input-mode 修飾キー転送の調査で使用 — § 15.13）。`/tmp/aish-debug.log` に書く `AISH_DEBUG`（Unix 前提）とは別系統で、**stderr 出力なので Windows でも `aish 2> pty.log` で取れる**。ConPTY のカーソル位置指定シーケンス調査用。
...
- **AI 応答ラベルは `[backend/model/effort]> `**（`print_ai_message`。中身は純関数 `format_backend_label`（golden test `backend_label_omits_none_fields`）が組む）。**model/effort は明示選択済み（config の `model`/`effort` / `/model`・`/effort` ピッカー / claude なら `extra_args` の `-m`）の欄だけを順に `/` で連結し、None の欄は省略する**（両方あり=`[claude/fable-5.1/high]`、model のみ=`[claude/fable-5.1]`、両方なし=`[claude]`）。この省略方針は**思考中スピナー `Spinner::start` と一致させる**こと。旧 `[ai/claude]>` から変更（ユーザ要件 2026-09: AI/model/effort を一目で分かるように）。**CLI 内部デフォルトは表示しない** — ユーザ未指定時 aish は `--model`/`--effort` を渡さず CLI が自前デフォルトを選ぶが、その値を aish は取得しない（問い合わせは passive 方針違反かつ headless JSON で確実に返す手段がない）。表示できるのは aish が把握している明示選択値のみ、という制約は意図的。**ring_buffer の注釈ラベル（`[ai/<kind>]>` = `record_ai_exchange`、§ 15.9）とは別物**で、そちらは AI へ再送する記録なので書式を変えない。
...
- **Claude の system prompt（`--append-system-prompt`）は初回ターンのみ**（resume での二重 append 回避）。内容は `system_prompt_rules`（役割・ツール不使用の安全制約・応答ルール）で、schema 非対応 backend 向けの JSON 単独出力指示（`JSON_OUTPUT_INSTRUCTIONS`）は付けない — 構造化出力として `--json-schema` が強制するため、prompt 側の「JSON だけを出力」指示は重複になる。**毎ターン守らせるルールは `--json-schema` の field description に書く**（スキーマは毎ターン送られる）。**この schema は `common::AI_RESPONSE_SCHEMA` に集約され claude と grok が共有**（grok も 2026-09 から `--json-schema` を使う。description の文言を直すときは両 backend に効くことに留意）。`command_result_followup`（§ 15.2）は Claude/grok では schema の **required に含めて毎ターン明示判定**させ、他 backend は `build_system_prompt` の応答ルール + JSON 例で指示し serde default（欠落=true）で吸収する。判定基準の文言は両経路で同一:「提案コマンドの実行後、その出力を見て分析・調査・操作を続行する必要があるなら true。ユーザにコマンドを教える・提示するだけで出力の確認が不要なら false」。`commands` description の整合性ルール: 「本文で実行コマンドを出したら配列にも入れる / 提案が無ければ空配列」（「空にするな」式の強制は付けない — 不要時にコマンドを捻り出す）+ 「独立した複数コマンドを `;` 連結せず配列の別要素に分割。**ただし `&&`/`||` と `for`/`while`/`until`/`case`/`if` 等制御構文内の `;` は 1 コマンド維持**」（分割すると `for i in 1 2 3; do …; done` が壊れる。codex 対策で共有 `build_system_prompt` にも併記）。
...
- **OpenCode（`opencode`、anomalyco/opencode = 旧 sst/opencode）は generic recipe として同梱**（2026-07、v1.17.13 / **v1.18.30 で再検証済み（2026-09）**）。read-only 強制の設計:
  ...
  - **agent 側 permission はトップレベル permission より優先される**仕様があるため、トップレベル deny だけでは project config の緩い agent に負けうる。対策として CONFIG_CONTENT 内に **deny 付き専用 agent `aish`** を定義し `--agent aish` で指定（同名 agent を project 側に定義されても CONFIG_CONTENT が後勝ち）。さらに **`task`（サブエージェント起動）/ `todowrite` / `skill` を `tools` で無効化**（task 経由で project 定義の緩い agent に迂回してファイル書き込みされる穴を塞ぐ。`skill` は v1.18.30 で deny 後も残っていたため 2026-09 に明示無効化＝skill を間接経路にした迂回も予防）。
  - 実機検証結果（scratch dir、`--auto` なし。v1.17.13 / v1.18.30）: deny された edit/bash は**ツールセット自体から除去**され（モデルが呼ぶと "unavailable tool"）、ファイル不変で拒否説明がテキストで返る。`skill` を明示無効化した後の残ツールは **glob/grep/read/webfetch/websearch のみ**（web 系は非 Claude 系 backend の意図的緩和方針どおり許可。モデルにツール名を列挙させて実測）。※非力な無料モデルは deny 済み操作を繰り返し試みて長時間ストールする場合があるが、書き込みは漏れない（aish は Ctrl+C で中断可）。
...
- **xAI Grok CLI（`grok` / `src/ai/grok.rs`）は Antigravity と同型の system-prompt-only native backend**（2026-08 追加。呼び出し名 = 実行ファイル名 `grok`）。lossy 抽出、内部 history 8 ターン。model は `-m`。
  ...
  - **構造化出力（`--json-schema`）を採用（2026-09）**。当初は gemini/qwen と同じ lossy 抽出だったが、公式 CLI が claude 同様 `--json-schema`（`--output-format json` を含意）で出力を schema 準拠に強制できると実測。**claude と同一の `AI_RESPONSE_SCHEMA`（`common.rs` に集約し両者で共有 = schema description の毎ターンルールが二重管理にならない）を渡す**。grok の封筒は `{ "text": "<schema 準拠 JSON 文字列>", "stopReason":…, "usage":… }` で、`parse_grok_response` が `text` を取り出し `AiResponse` にパース。**grok は従来 lossy backend だったので hard error にせず段階フォールバック**（text→AiResponse / text を lossy / 封筒直下 / stdout 全体を lossy）。system prompt（`build_system_prompt`）は read-only posture のツール禁止指示に必須なので据え置き（schema と同形の JSON 指示なので矛盾しない）。
  - resume は CLI に `-r/--resume`・`-c/--continue` が存在するが**使わず内部 history で代替**（backend 横断の統一方針。`resume_command` は None のまま）。**この CLI は `--permission-mode plan`/`--sandbox`/`--deny` で read-only 強制も可能だが、native は system-prompt-only 許容の既存合意（下記）に従い採用しない**（plan モードは headless でハングせず書き込みを実弾拒否することは実測済み。将来 read-only 強化する場合の候補）。
...
- **claude headless の失敗 stderr は `[claude-code:<tag>] {json}` マーカーを含む**（例: `[claude-code:unrecognized_model] {"model":"Opus 5","query_source":"sdk"}`）。aish が `--model` を渡していなくても `~/.claude/settings.json` の `"model"` に表示名（"Opus 5" 等）が保存されていると headless (query_source=sdk) は slug 解決できず失敗する — 対話 UI とは解決経路が違う（2026-08 実障害。「limit では」と誤解された）。`conversation::format_ai_error` が `claude_error_tag`（純関数。tag は英数字+`_` のみ受理し引用文での誤検出を防ぐ）で抽出し、`Claude Error: <tag>` 見出し + stderr 原文 + `claude_error_hint` の種類別ヒント（`model` 系 → `/model`・settings.json 確認 / `auth`・`login`・`oauth`・`api_key`・`credential`・`billing` 系 → 再ログイン / `limit`・`quota`・`overloaded` 系 → limit 待ち）で表示。**未知 tag はヒント無しで原文のみ**（的外れな誘導をしない）。マーカー無しのエラーは従来の汎用文言（login or usage limit）を維持。いずれも golden test 対象。
  - **backend 名取り違えヒント（`backend_name_confusion_hint`、2026-09）**: `aish --model grok` のように **backend 名を `--model` に渡す**と、backend は既定 claude のままモデル名だけ `grok` になり claude が `unrecognized_model` を返す（`--ai` と `--model` の混同。実ユーザが遭遇）。対策として model エラー本文の `{"model":"<v>"}` を parse し、`<v>` が `BackendKind::all_native()` の名前と一致（大小無視）したら見出し・種類別ヒントに加えて `Hint: "<v>" is an AI backend name, not a model. Use \`aish --ai <v>\` instead of \`--model <v>\`.` を追記する純関数。**generic backend 名（opencode 等）は registry init 前で列挙できないため native 名のみ対象**。golden test `backend_name_confusion_hint_detects_model_as_backend`。
...
- **ConPTY の内部仮想スクリーンと実端末のズレによる表示崩れ → ConPTY 再同期（`conpty_sync`）で解決（2026-08、詳細は § 15.14）**。aish は AI メッセージ/提案コマンド/`Exec?` 確認行を PTY を経由せず直接 stdout に書く設計だが、Windows ConPTY は子プロセス (PowerShell) 向けに実端末とは別の内部仮想スクリーンバッファを保持しており、**aish が直接書いた分だけ実画面がスクロールしても ConPTY はそれを一切知らない**。その状態で PSReadLine のコマンドエコー再描画（絶対座標 CUP）や打ちかけ消去（`\x1b[行;列H\x1b[nX`）が届くと、ずれた行に描画され aish の出力（AI 応答等）を上書きする。2026-07 時点では「根本修正には自前の画面モデルが必要」として見送っていたが、2026-08 に**境目での再同期方式**（aish が描いた行を anchor 行より上へ退避して cursor を ConPTY モデルに合わせ直す）で解決した。残存する劣化モード: 端末リサイズ / `cls` 等で ConPTY が全面再描画すると、画面上に見えている aish 出力は上書きされる（scrollback 分は残る）。
- **落とし穴（2026-08 実測、`portable_pty` プローブで確定。同月に根因を更新）: 子 ConPTY への合成キー（kill_line の ESC / refresh_prompt・空 Enter 注入の Enter）は「素のバイト」では確実に届かない**。当初は「ESC と `\r` が 1 バーストで届くと Alt+Enter に解釈される」race として 50ms 分離で対処したが（probe 実測: 分離で `\r\nPS...>` 再表示が安定 / 1 write の `\x1b\r` は何も出ない）、その後さらに **パススルーが win32-input-mode のキーイベント列を 1 つでも子 ConPTY に転送すると、以後は素の単独 ESC / 単独 `\r` がタイミングに関係なく全て無視される**ことが判明した（probe 実測: 確認キー 'y' の key-up `\x1b[89;21;121;0;0;1_` の転送有無だけで再現/消失。down+up ペアでも同様に毒になる。テキスト + 末尾 `\r` のコマンド送信は影響を受けない）。aish では `read_confirm_key` が 'y'/'A' の key-down を消費した後、対応する key-up が `wait_for_command_completion` の生 stdin 転送で必ず PTY へ流れるため、**2 回目以降の refresh_prompt が全て空振り → ConPTY モデルのプロンプト行が進まず、次のコマンドエコー（絶対座標 CUP）が実画面のプロンプトの 1 行上の空行に「` > コマンド`」だけ浮いて描かれる 1 行ズレ**（2026-08 ユーザ報告の実体）になっていた。**対策**: 子が `\x1b[?9001h`（win32-input-mode）を要求した後は、aish の合成キーも win32-input-mode シーケンス（ESC=`\x1b[27;1;27;Kd;0;1_`、Enter=`\x1b[13;28;13;Kd;0;1_`、各 down+up）でエンコードして送る（probe 実測: 毒状態でも常に届く）。検出は `pty_drain::drain_pty`（全 PTY 出力の choke point）→ `pty_handler::note_pty_output` が `\x1b[?9001h/l` をチャンク境界の持ち越し (8 byte) 付きで走査し `WIN32_INPUT_MODE` フラグを更新する。**`\x1b[?9001h` を出さない conhost（旧環境）向けに素の ESC + 50ms 分離 + `\r` の旧経路を fallback として維持**（`refresh_prompt` の sleep は fallback 側のみ）。`send_approved_command` のコマンド文字列 + 末尾 `\r` は素のままで問題ない（実測。変える場合は再実測してから）。
- **落とし穴（2026-08 実測、E2E ハーネスで根因特定）: win32-input-mode の修飾キー単体イベントを PTY へ転送すると子シェルが「Ctrl 押しっぱなし」になる**。Windows Terminal / conhost は win32-input-mode で修飾キー自体の down/up（`\x1b[17;29;0;1;…_` = VK_CONTROL down 等、uChar=0）もシーケンスとして送る。従来 aish はこれを `Tok::EscSeq` として **raw のまま子へ転送**していたが、Ctrl+/ エントリでは **Ctrl-down だけが転送され、対応する Ctrl-up は minibuffer の読み取りループに飲まれて転送されない**。子 ConPTY は Ctrl 押下状態のままになり、以後 aish が送る plain な ESC / `\r`（kill_line / refresh_prompt / send_approved_command の Enter）が **Ctrl 修飾扱いで無視される**（probe 実測: Ctrl-down シーケンス注入後は ESC+50ms+CR でもプロンプト再表示が出ない）。対策: `classify_win32_input_mode` が修飾キー単体イベント（VK 0x10-0x12/0x14/0x5B/0x5C/0x90/0x91/0xA0-0xA5、down/up とも）を `Tok::Win32Modifier` に分類し、**passthrough が破棄して転送しない**。文字キーのシーケンスは自身の Cs フィールドで修飾状態を運ぶため、修飾キー単体イベントを落としても子の Ctrl+C 等の解釈は壊れない（劣化モード: 子内の TUI が「修飾キー単体の押下」を追跡する稀なケースでは届かなくなる）。
- **実端末 cursor 位置は Windows では Console API（`term::cursor_position`、`GetConsoleScreenBufferInfo`）で取る**。DSR（`\x1b[6n`）は応答が stdin 経由で届くため、win32-input-mode 環境で応答断片が minibuffer 入力に混入・タイムアウトで fallback に落ちる事故があった（2026-08 E2E 実測: minibuffer 入力欄に `;1_` が混入 + fallback 座標で conpty_sync の anchor が不正確になり再同期が空振り）。`show_minibuffer` は `cursor_position()` を優先し、None（Unix）のとき DSR に fallback する。Unix 実装は追加せず（`term/mod.rs` の cfg スタブが None を返す = conpty_sync が Unix で自然に no-op になる意図的な設計）。
...
### 15.14 ConPTY 再同期（conpty_sync）と Windows ローカル起動バースト

Windows で「aish が直接 stdout に描いた行が、後続のコマンドエコー・プロンプト再表示・コマンド出力に上書きされる」問題（旧 § 15.8 Bug 2 + 起動バナー消失）の解決機構（2026-08）。実測データは `portable_pty` で PowerShell / aish 本体を ConPTY 配下に起動するプローブ / E2E ハーネスで採取した。

- **前提（実測で確定した ConPTY の挙動）**: 子シェルの出力は一旦 ConPTY 内部のスクリーンバッファに描画され、ConPTY がそれを再合成した VT が aish に届く。通常のシーケンシャル出力・空 Enter のプロンプト再表示は**相対**（`\r\n` + テキスト）だが、**PSReadLine のコマンドエコー再描画は絶対座標 CUP**（`\x1b[6;26H...`）、**打ちかけ消去は絶対座標 CUP + ECH**（`\x1b[24;26H\x1b[13X`）で届く。また **spawn 直後に全画面クリア + 初回プロンプト**（`\x1b[?25l\x1b[2J\x1b[m\x1b[H` + `PS ...>`）を必ず emit する。
- **問題の構造**: aish の直接描画（minibuffer スクロール・`[aish]` エコー・AI 応答・`Exec?` 行）は ConPTY の関知外なので、実画面だけがスクロールして ConPTY 内部モデルと行ズレする。ズレたまま PTY 表示を再開すると、上記の絶対座標描画が「モデル基準の行」= aish の出力の上に当たる。Unix は bash の redisplay が相対シーケンスなので起きない。
- **解決 = 境目での再同期（`src/conpty_sync.rs`）**: PTY と実画面が同期している瞬間（minibuffer 入口 / AI 提案コマンド完了時）の cursor 位置を **anchor** として記録し（`set_anchor` / `capture_anchor`、Windows は Console API で取得）、aish 直接描画 → PTY 表示再開の境目で `resync(pty, pty_rx, ring_buffer)` を呼ぶ。合わせ方は 2 方式（2026-08 に注入方式へ変更。ユーザ要件「スクロール位置を変えない」）:
  - **空 Enter 注入（ローカルシェル、既定）**: N = 実 cursor 行 − anchor 行 個の空 Enter を子シェルへ送り、そのプロンプト再表示出力を非表示で吸収（記録は ring_buffer へ）して **ConPTY モデル側の cursor を実画面の行まで進める**（起動バナー整合と同じ手法）。実画面へは何も書かないため**ユーザの見ているスクロール位置は一切動かない**（旧方式の全画面 LF 退避は resync のたびに画面がジャンプした）。直後の `refresh_prompt` の表示（`\r\n`+プロンプト）が実画面では cur+1 行目・モデルでは anchor+N+1 行目に描かれ、N = cur − anchor で一致する。**画面下端の飽和も同式のまま**（実画面が自然スクロールした分だけ cur が頭打ちになり、注入側もモデル下端で内部スクロールして双方同じだけ動く。E2E 4 ターン目で検証済み）。cur ≤ anchor（minibuffer キャンセルで cursor が上に復元された等）は注入では戻せないため cursor を anchor へ絶対復帰するだけ。注入の許可は `set_empty_enter_injection(mode == Local)`（main::run が spawn 時に 1 度設定）。
  - **全画面 LF スクロール退避（リモート = SSH の fallback、`resync_scroll`）**: リモートにはサーバへ入力を注入しない原則のため従来方式を維持: aish が描いた行を anchor 行より上へ退避（scrollback に残る）→ cursor を anchor へ絶対復帰（この方式は画面がスクロールする）。
  以降 ConPTY の絶対座標と実画面が行単位で一致するのでパススルーは無加工のまま。**コマンド変形はしない。空 Enter は入力行が空の状態でのみ送られ、何も実行せず履歴にも残らない（信頼の根幹は不変）**。Unix では `term::cursor_position()` が None を返すため全て no-op。
- **呼び出し箇所**: ① `confirm_and_execute` の各コマンド送信前（`resync()` が true = aish が何か描いた場合は初回以外でも `refresh_prompt` + 150ms 待ち + drain 表示でプロンプトを描かせてからコマンドを送る — エコーが「プロンプトの無い行」に浮くのを防ぐ + 完了 sniffer が refresh のプロンプトを誤検出しない）② `run()` 末尾の終了 refresh 前 ③ main loop の MinibufferCancelled ④ slash command 表示後。anchor 更新は minibuffer 入口 + `wait_for_command_completion` のプロンプト復帰時。リサイズ時は `clear_anchor()`（ConPTY が全面再描画して対応が変わるため。次の minibuffer で再記録）。
- **`run()` 末尾 drain の `skip_leading_newline` は `cfg!(unix)` 限定**: Windows で先頭 `\r\n` を表示から畳むと、resync 後の実画面だけが 1 行上にずれて以降の絶対座標描画が全て 1 行ずれる（off-by-one）。
