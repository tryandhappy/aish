...
- **実機検証チェックリスト（未了。Windows GA の前提）**:
  ...
  2. Ctrl+/ で minibuffer が開く（US/JIS 配列）、矢印・Alt+Enter・IME 日本語確定（サロゲート含む）・ペースト
