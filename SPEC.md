...
| `/effort [LEVEL]` | reasoning effort を runtime 変更（次回 send 以降）。**引数なし=候補ピッカー**（§15.12）、`-`/`clear`=クリア、その他=検証せず set。gemini/qwen/cursor は保存のみ、claude/codex/copilot は native 反映 |
| `/model [NAME]` | モデルを runtime 変更（session_id / history 維持）。**引数なし=候補ピッカー**、`-`/`clear`=クリア、その他=検証せず set |
...
各 backend テーブル（`[ai.claude]` / `[ai.codex]` / `[ai.gemini]` / `[ai.qwen]` / `[ai.cursor]` / `[ai.copilot]` と `[[ai.providers]]`）に `#[serde(flatten)]` で `OptionLists` が埋め込まれ、`/model` `/effort` の対話ピッカー候補を決める（§15.12）。

| キー | 既定値 | 説明 |
|---|---|---|
| `models` | `[]` | model 候補 (static)。空なら `models_command` / 組み込み既定にフォールバック |
| `models_command` | `""` | model 候補取得コマンド。ローカルで `sh -c`、stdout を 1 行 1 候補で解釈 |
| `efforts` | `[]` | effort 候補 (static) |
| `efforts_command` | `""` | effort 候補取得コマンド |
...
- **Cloudflare Workers AI backend（`cloudflare` / `src/ai/cloudflare_workers_ai.rs`）は REST を `curl` サブプロセスで叩く native backend**。aish は HTTP/TLS クレートを持たず `--update` も curl 方式（§15.11）なので、CF も同方針＝**追加 crate 依存ゼロ**で `run_cli_capture_stdout("curl", …)` を使い、Ctrl+C 中断・ログ・確認フローを再利用する（`binary()`="curl" なので `check_installed` は `curl --version` を見る＝真の実行時依存は curl）。session/resume 機構は無く gemini/qwen と同型の内部 history を毎ターン前置きする。
  - **認証は環境変数のみ**（`CLOUDFLARE_ACCOUNT_ID` / `CLOUDFLARE_API_TOKEN`、任意で `CLOUDFLARE_MODEL`）。config.toml は平文なのでトークンを置かせない（`[ai.cloudflare-workers-ai]` は `/model` 候補の `OptionLists` だけ）。**呼び出し名は `cloudflare`、設定セクション/ファイル名は `cloudflare-workers-ai` 系で住み分け**（Rust モジュールはハイフン不可で `cloudflare_workers_ai.rs`、TOML は serde `rename = "cloudflare-workers-ai"`）。
  - **curl に `-f`(`--fail`) を付けない**：Cloudflare は HTTP エラー時も JSON のエラーボディ（`success:false` + `errors`）を返すので、それを読んでメッセージ化したい。レスポンスは `success` を確認し `result.response`(文字列) を取り出して `parse_ai_response_lossy` に渡す（モデルが `{message, commands}` JSON を吐けばコマンド提案化、吐かなければ全文 message）。body は `--data-binary @-` で stdin 経由（ARG_MAX / クォート回避）。
  - **既知トレードオフ**: `Authorization: Bearer <token>` を `-H` 引数で渡すため token が argv に乗り `ps` で見え得る（ローカル・本人資格情報なので許容。気になれば後続で `-H @-` / 一時ファイル方式へ）。コマンド提案の質はモデル依存で、`{message,commands}` フォーマットに追従できる大きめ instruct モデル推奨。テキスト生成のみでサーバ側は一切実行しないので信頼の根幹は不変（提案は従来どおり Y/n/a/q でゲート）。
...
- **`ui::show_picker` は confirm（`read_confirm_key`）と同型: main スレッドが fd0 を直接読む同期ブロッキング関数**。`/model` `/effort` は main loop の `AiPrompt` arm 内（入力スレッドが `rearm_on_drop` guard で parked 中）で処理されるので、minibuffer のような `InputEvent`/`InputRequest` チャネル経由にせず直接呼ぶ。**termios は触らない**（raw はセッション維持）。描画は stdout 専用で PTY に書かない。表示中は同スレッドが drain しないので PTY が画面を上書きすることはなく、専用の抑制フラグも不要。
...
- **`/model` `/effort` ハンドラ（`run_option_picker`）は常に `Some(...)` を返す**（None だと通常 AI プロンプト扱いになる）。引数なし=候補解決→空なら hint / 非空ならピッカー（末尾に `(default)` 疑似エントリ＝選べばクリア）、`-`/`clear`=クリア、その他=**検証せず set**（ヒントのみ。一覧外の値も許可）。取消時は変更しない。
