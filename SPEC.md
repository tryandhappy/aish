...
- **kill_line / refresh_prompt**（pty_handler.rs）: Windows は `0x01,0x0b` でなく **ESC(0x1b)** を送る。PSReadLine の Windows edit mode では Ctrl+A=SelectAll 等で kill-line にならず、**打ちかけ未消去のまま改行 = 未承認 submit（信頼の根幹違反）** になり得るため。ESC は cmd.exe / PSReadLine 共通の入力行クリア。バイト列は `KILL_LINE_BYTES` 定数として pty_handler.rs 内に維持（リテラルを外に出さないルール続行）。**落とし穴（2026-07 実機報告 + `AISH_DEBUG_PTY` ログで確認）**: `confirm_and_execute`（conversation.rs）が最初の AI 提案コマンド実行直前に送る `kill_line()` の単独 ESC の**直後に間を置かずコマンド文字列を送信すると**、送信バイト列が `ESC` + コマンド本体になり、コマンドが `[` で始まる場合（`[System.Environment]::OSVersion.Version` 等）PowerShell の VT 入力デコーダが `ESC` `[` 直後の 1 バイトまでを 1 個の CSI シーケンスとして消費してしまい、**承認したコマンド文字列の先頭 1-2 文字が欠落したまま実行される**（実測: `[System.Environment]::...` → `ystem.Environment]::...` が実行され `CommandNotFoundException`）。ユーザが画面で承認した文字列と実際に PowerShell が解釈する文字列が食い違う trust 上の問題。対策: `kill_line()` 送信後に `KILL_LINE_REDISPLAY_WAIT`(150ms) だけ待ってから `send_approved_command` を送る（ESC が単独キーとして確定する猶予を与える）。Unix は kill_line が `0x01,0x0b` でこの衝突が起きないが、分岐を増やさず両 OS で同じ待ちを入れている。
