...
- **落とし穴（2026-08 実測、`portable_pty` プローブで確定）: ESC と Enter (`\r`) が子 ConPTY に 1 バーストで届くと `ESC \r` が Alt+Enter として解釈され両方無視される**（打ちかけ消去もプロンプト再表示も起きない。到着タイミング依存の race で、pipe 書き込みが別でも子側 read で結合されると発生）。`refresh_prompt` は ESC 書き込み後 `cfg!(windows)` で 50ms sleep してから Enter を送り、ESC を単独キーとして確定させる（pty_handler.rs 内に固定。probe 実測: 50ms 分離で `\r\nPS...>` の再表示が安定して出る / 1 write の `\x1b\r` は何も出ない）。
...
- **解決 = 境目での再同期（`src/conpty_sync.rs`）**: PTY と実画面が同期している瞬間（minibuffer 入口 / AI 提案コマンド完了時）の cursor 位置を **anchor** として記録し（`set_anchor` / `capture_anchor`、Windows は Console API で取得）、aish 直接描画 → PTY 表示再開の境目で `resync()` を呼ぶ: aish が描いた行（anchor 行以降に伸びた分）を**全画面 LF スクロールで anchor 行より上へ退避**（minibuffer の伸長と同じ方式 = scrollback に残る）→ cursor を anchor へ絶対復帰。以降 ConPTY の絶対座標と実画面が行単位で一致するのでパススルーは無加工のまま。**挿入するのは空行スクロールと cursor 移動だけで、入力注入もコマンド変形もしない（信頼の根幹は不変）**。Unix では `term::cursor_position()` が None を返すため全て no-op。
- **呼び出し箇所**: ① `confirm_and_execute` の各コマンド送信前（`resync()` が実画面をスクロールした場合は初回以外でも `refresh_prompt` + 150ms 待ち + drain 表示でプロンプトを描かせてからコマンドを送る — エコーが「プロンプトの無い行」に浮くのを防ぐ + 完了 sniffer が refresh のプロンプトを誤検出しない）② `run()` 末尾の終了 refresh 前 ③ main loop の MinibufferCancelled ④ slash command 表示後。anchor 更新は minibuffer 入口 + `wait_for_command_completion` のプロンプト復帰時。リサイズ時は `clear_anchor()`（ConPTY が全面再描画して対応が変わるため。次の minibuffer で再記録）。
...
- **UX 上のトレードオフ（許容済み）**: プロンプトが画面上部にある状態（`clear` 直後等）で AI 応答が長いと、再同期時に応答が scrollback 側へ押し上げられ画面がジャンプする（内容は失われない。旧挙動は上書き破壊だったので厳密に改善）。anchor 行は 1 行の空きとして残る。
- **Windows ローカル起動バースト（`main::windows_local_startup`）**: ConPTY の初期全画面クリアで spawn 前バナーが必ず消えるため、Windows + ローカルシェルではバナーを後から描く。初期バーストを表示せず吸収（`DrainOpts::capture` で生バイトも捕獲）→ `conpty_sync::startup_burst_is_clean`（全画面クリア + プロンプト 1 行だけか）を判定 → クリーンなら: 端末状態シーケンスだけ転送（`filter_terminal_state`: `\x1b[?...h/l` と OSC を通しテキスト/CUP/ED 等を捨てる。win32-input-mode 要求 `\x1b[?9001h` を捨てると入力経路が変わるため）→ 実画面を全画面 LF スクロールで scrollback へ退避して原点からバナー描画 → **バナー行数ぶんの空 Enter（`\r`）を子シェルへ注入**して ConPTY frame のプロンプト行をバナーの下の行位置まで進め（出力は非表示・記録のみ。空 Enter は `refresh_prompt` が常用する `\r` と同じ無害な入力で、履歴にも残らない。**ローカルシェル限定 — リモートには送らない**）→ 最後にもう 1 回 `\r` を送りそのプロンプト再表示だけを表示する。これで初回プロンプトから ConPTY 座標と実画面が一致する。profile 出力等でバーストがクリーンでない場合はバナー描画 → バーストそのまま表示（従来動作 = バナーはクリアで消える）にフォールバック。バーストが 4 秒来ない場合もバナーだけ描いて通常フローへ。
