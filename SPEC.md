...
- **Cloudflare Workers AI backend（`cloudflare` / `src/ai/cloudflare_workers.rs`）は REST を `curl` サブプロセスで叩く native backend**。aish は HTTP/TLS クレートを持たず `--update` も curl 方式（§15.11）なので、CF も同方針＝**追加 crate 依存ゼロ**で `run_cli_capture_stdout("curl", …)` を使い、Ctrl+C 中断・ログ・確認フローを再利用する（`binary()`="curl" なので `check_installed` は `curl --version` を見る＝真の実行時依存は curl）。session/resume 機構は無く gemini/qwen と同型の内部 history を毎ターン前置きする。
  - **認証は環境変数のみ**（`CLOUDFLARE_ACCOUNT_ID` / `CLOUDFLARE_API_TOKEN`、任意で `CLOUDFLARE_MODEL`）。config.toml は平文なのでトークンを置かせない（`[ai.cloudflare-workers]` は `/model` 候補の `OptionLists` だけ）。**呼び出し名は `cloudflare`、設定セクション/ファイル名は `cloudflare-workers` 系で住み分け**（Rust モジュールはハイフン不可で `cloudflare_workers.rs`、TOML は serde `rename = "cloudflare-workers"`）。
