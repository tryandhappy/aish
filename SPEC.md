...
- **スコープ外（既知の制約）**: `--update` 自己更新は `Unsupported platform` を維持（実行中 exe の rename 不可・sha256sum 不在など独立課題）。リリースへの Windows バイナリ（x86_64/aarch64 の生 exe + zip）は `release.yml` の matrix に同梱済み（2026-07 実機検証完了後に追加）。Ctrl+Break は console ctrl event として aish を即殺し得る（必要なら SetConsoleCtrlHandler で無視、初期は文書化のみ）。legacy conhost / mintty は対象外（`console_ok()` が拒否）。
