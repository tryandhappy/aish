...
- **Enter のデフォルトは常に Yes**（このコマンド 1 件だけ実行。2026-09 変更。2026-08 の「複数時 Enter=All」を撤回 = ユーザ要望「意図しない一括承認を避けたい」）。残コマンドあり = プロンプト `[Y/n/e/a/q]`（Y 大文字=デフォルト、a 小文字）、最後／単一 = `[Y/n/e]`。**残り自動承認 (All) は明示的に `a`/`A` を押してのみ選ぶ**（Enter には載せない = 安全側）。echo は常に `Y`。Space も文脈に依らず Yes。**`default_all` フラグは廃止**（`InputRequest::ReadConfirmKey` はフィールド無し）。
...
AI backends（§ 15.10）:
- **Claude: 毎ターン守らせるルールは system prompt でなく `--json-schema` の description に書く**（system prompt は初回のみ）。**schema は `common::AI_RESPONSE_SCHEMA` に集約し grok と共有**（description を直すと両方に効く）。独立コマンドは `;` 連結せず `commands` 配列に分割（`&&`/`||`・制御構文内の `;` は維持）。
...
- **`cargo install --path .`（`~/.cargo/bin/aish` への配置）はしない**（`/release` スキル § 5 の install 提案も常に No）。ユーザ環境の実体は `/usr/local/bin/aish` で、`~/.cargo/bin/aish` と二重になるとデバッグでどちらが動いているか紛れて不便なため（ユーザ指示 2026-09）。ローカルに dev ビルドを入れて検証したい場合は `/usr/local/bin/aish` を上書き（`cargo build --release && sudo install -m 755 target/release/aish /usr/local/bin/aish`）する。
