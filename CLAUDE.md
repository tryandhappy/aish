...
- `ui.rs` - ターミナル制御。rawモード(セッション全体で維持)、行編集、パススルーモード（Ctrl+/ミニバッファ）、ANSI色
...
## ユーザ入力
- `Ctrl+/`: ミニバッファを開いてAI呼び出し。ターミナル最下行に[aish]プロンプトを表示し、入力確定でAI呼び出し。Ctrl+Cでキャンセル。空Enterは無操作
- `? <prompt>` / `@ai <prompt>`: RemoteEndedモードのReadLineでのみ有効（parse_inputで処理）
- それ以外: PTYにパススルー（タブ補完等そのまま動作）
- Ctrl+C: パススルーモードではPTYにそのまま送信（record_ctrl_cしない）。ReadLineモードでは連打でaish終了
- ミニバッファ中はPTY出力の画面表示を抑制（MINIBUFFER_ACTIVE フラグ）。リングバッファへの記録は継続
