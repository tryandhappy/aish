...
- win32-input-mode（`ESC[Vk;Sc;Uc;Kd;Cs;Rc_`）は `input.rs` の `classify_win32_input_mode` でデコード（`classify_csi` が final byte `_` で呼ぶ純関数。`#[cfg]` を付けず golden test 対象）。Windows Terminal + PowerShell では PSReadLine がこのモードを有効化し、Ctrl+/ 等が KEY_EVENT でなくこのシーケンスで届くため（付けないと Ctrl+/ が PowerShell に素通りしミニバッファが開かない）。key-down のみ Tok 化、key-up/解釈不能は None→EscSeq に握りつぶす（二重入力防止）。raw は保持したままなので passthrough は生バイトを PowerShell/子 TUI へ転送し復号させる（透明性維持）。例外: 修飾キー単体イベント（Ctrl/Shift/Alt 等、down/up とも）は `Tok::Win32Modifier` にして passthrough が破棄・転送しない（転送すると Ctrl+/ エントリで down だけ子へ届き「子シェルが Ctrl 押しっぱなし」になり、以後の kill_line/refresh_prompt の ESC/`\r` が Ctrl 修飾扱いで無視される — § 15.13。文字キーは自身の Cs フィールドで修飾を運ぶので破棄しても壊れない）。

確認プロンプト Y/n/e/a/q（§ 15.2）:
- 1 キー即確定（`read_confirm_key()`）。Space=実行(Yes)、`n`=1 回スキップ、`a`/`A`=残り自動承認、`q`=残り中止(follow-up あり)、Ctrl+C/Ctrl+D=残り中止(AI に問わない)、ESC=`n`。未知キーは無視して再読み取り。ESC を Ctrl+C 系 abort に戻さない。
- Enter のデフォルトは常に Yes（このコマンド 1 件だけ実行。意図しない一括承認を避けるため）。残コマンドあり = プロンプト `[Y/n/e/a/q]`（Y 大文字=デフォルト、a 小文字）、最後／単一 = `[Y/n/e]`。残り自動承認 (All) は明示的に `a`/`A` を押してのみ選ぶ（Enter には載せない = 安全側）。echo は常に `Y`。Space も文脈に依らず Yes。`InputRequest::ReadConfirmKey` はフィールドを持たない（Enter の意味を文脈で切り替えるフラグを足さない）。
...
- AI 応答の `command_result_followup: false` は実行後の AI 自動問い合わせを抑制（`q` でも抑制、欠落時 true = 従来動作）。判定基準は Claude schema description と `build_system_prompt` の両方に同一文言で記述（片方だけ直さない）。
- 制御文字ガードは `VettedCommand` 型。表示・送信が `&VettedCommand` のみ受理し「承認した物 = 実行する物」を型で保証（撤去・迂回は型エラー）。
- AI 応答の `commands` は `ProposedCommand{command,risk}` の配列（`ai::types`）。後方互換で裸文字列も受理（custom `Deserialize`。裸文字列/risk 欠落/不明値/未知キー(旧 explanation 等)は無視 or `Risk::Yellow` 既定 = 安全側）。`risk` は表示専用 metadata で `VettedCommand`・PTY 送信バイトには一切入れない（信頼境界。loop index で並走、`e=編集`後も元の色を維持）。risk 4 分類（Green/Yellow/Orange/Red）は AI 由来で、判定基準を `AI_RESPONSE_SCHEMA` の risk description と `build_system_prompt` の応答ルールに同一文言で記述（片方だけ直さない = §15.10。drift ガードテスト `schema_and_prompt_share_risk_taxonomy` あり）。確認プロンプトはコマンド本文だけを危険度色（`ui::risk_color`=green108/yellow178/orange166/red196、ハードコード。ネオン原色を避けトーンダウン、緑はさらに彩度を落としたセージ 108、橙はブランド色 208 と区別し 166）で描画、`Exec?` ラベルは明黄+灰背景（`38;5;228;48;5;239`。aish プロンプトのオレンジ+暗茶背景とは背景色で、confirm ブラケットの淡青とは前景色で区別）・`[Y/n/e/a/q]` ブラケットは confirm_color（既定 `38;5;153` 淡青+灰背景）を維持（`build_confirm_prompt` 純関数、golden test）。`print_ai_commands` の一覧も危険度色。コマンドごとの説明文は付けない（表示が煩雑になるため。schema/prompt にも explanation を持たせない）。
...
- 候補解決は backend ごと `available_models`/`available_efforts` = static list > 取得コマンド > 組み込み既定（`common::resolve_option_list`）。取得コマンドはピッカーを開く時だけローカル実行。effort 既定は claude/codex/copilot のみ、model 既定は全 native backend（generic は recipe 由来のみ）。
- モデル一覧コマンドはほぼ無いので動的取得は原則不可（例外: cursor は `cursor-agent models` を持つが要 auth・出力形式未実測のため組み込み自動取得はせず `models_command` 例に留める — § 15.12）。CLI 側に「最新へ解決するエイリアス」がある backend だけ `MODEL_DEFAULTS` 先頭にそれを並べて陳腐化を防ぐ（§ 15.12）。適用済み: claude(`default`/`opus`/`sonnet`/`haiku`/`fable`) / cursor(`auto`) / qwen(`qwen3-coder-plus`/`flash` ローリング tier)。これら先頭のエイリアスは消さない。grok の `MODEL_DEFAULTS` に `-latest` エイリアスを置かない（xAI の `-latest` は modelname 単位でしか解決せず、改番で陳腐化回避に効かないため）。codex/copilot/gemini/antigravity/REST 2 種も最新解決エイリアスが無く（codex は `(default)` 未指定で代替、gemini `-latest` は experimental で不採用、copilot の auto は headless 可否未実測、REST はモデル ID 直渡し）、`MODEL_DEFAULTS` は best-effort スナップショットのまま = 陳腐化回避は `(default)` エントリ / `models_command` / リリース更新に委ねる。スナップショットの更新内容は git log、backend ごとの据え置き理由は SPEC.md § 15.12。
...
AI backends（§ 15.10）:
- Claude: 毎ターン守らせるルールは system prompt でなく `--json-schema` の description に書く（system prompt は初回のみ）。Claude の system prompt に JSON 単独出力指示を入れない（`build_system_prompt_claude` = `system_prompt_rules`、出力形式は `--json-schema` が強制。JSON 指示付きの `build_system_prompt` は schema 非対応 backend 用）。schema は `common::AI_RESPONSE_SCHEMA` に集約し grok と共有（description を直すと両方に効く）。独立コマンドは `;` 連結せず `commands` 配列に分割（`&&`/`||`・制御構文内の `;` は維持）。
...
- 組み込みデフォルト recipe は `config::builtin_providers()` 1 関数に集約し、安全フラグを `args` に焼き込んで同梱（generic 原則の例外）。read-only/plan 相当を強制できない CLI は同梱しない。ユーザ上書きは同名エントリのフィールド単位マージ（`args`/`env` は丸ごと置換）。registry へ渡すのは `resolved_providers`。
- **OpenCode（`opencode`）は generic recipe として同梱**。read-only は `OPENCODE_CONFIG_CONTENT` env 注入（recipe の `env` フィールド → `run_cli_capture_stdout_env`）+ deny 付き専用 agent `aish`（task/todowrite/skill も無効化）で強制（v1.18.30 で再検証済み。残ツールは glob/grep/read/webfetch/websearch）。**`--auto` は絶対に付けない / `--agent plan` に変えない**（plan agent は ask ベースで headless ハング。経緯は SPEC.md § 15.10）。
...
- AI CLI 失敗表示は `conversation::format_ai_error` に集約。claude の `[claude-code:<tag>]` stderr マーカーは `claude_error_tag`/`claude_error_hint`（純関数 + golden test）で `Claude Error: <tag>` 見出し + 種類別ヒントに分岐。未知 tag はヒント無しで原文のみ、マーカー無しは従来の汎用文言（login or usage limit）を維持。model エラー本文の `{"model":"<v>"}` が native backend 名と一致したら `backend_name_confusion_hint` が `--ai <v>` を追加提案（`--model grok` のように backend 名を `--model` に渡す取り違え対策。generic 名は init 前で列挙不可のため native のみ）。
- **Antigravity (`antigravity`) / Grok (`grok`) は gemini/qwen と同型の system-prompt-only native backend**（`src/ai/antigravity.rs` / `src/ai/grok.rs`）。read-only/plan の permission-layer 強制は headless で無いが、**native backend は歴史的に system-prompt-only 姿勢を許容**（read-only 強制必須は builtin generic recipe の話。信頼の根幹＝サーバ保護はコマンド承認 UI が担保し backend の read-only 有無に非依存、というユーザ合意 2026-08）。`agy -p`/`grok` を stdin 渡し・lossy parse・内部 history。**`--dangerously-skip-permissions`/`--always-approve`/`--permission-mode bypassPermissions` 等の auto-approve 系は絶対に付けない**。Antigravity: 呼び出し名 `antigravity`・実行ファイル `agy`（`binary()` で分岐）・model `--model`・effort `--effort`（native）・resume `agy --continue`・auto_detect は Gemini 直後。Grok（公式 xAI CLI `grok` v1.0.25 で実測、2026-09）: 呼び出し名=実行ファイル `grok`・**prompt は `grok --prompt-file /dev/stdin`（Unix、複数行/ARG_MAX 安全）/ `-p <prompt>` 引数（Windows、`/dev/stdin` 不可のため）。`-p` 単独＋stdin は不可**（公式の `-p/--single` は PROMPT を引数値として要求し stdin を読まない＝旧実装のバグ）・model `-m`・**effort `--reasoning-effort`（別名 `--effort`、値 `low/medium/high/xhigh`、`EFFORT_DEFAULTS`）を send で適用**・**model 一覧は `grok models` を実行してパース（`parse_grok_models`=`*`/`-` 行の先頭トークン抽出、未ログイン/失敗は `MODEL_DEFAULTS` に fallback）**・**出力は claude と同じ `--json-schema`（`common::AI_RESPONSE_SCHEMA` を共有）で構造化強制し封筒 `text` を `parse_grok_response` でパース（失敗時 lossy fallback）。schema description を直すときは claude/grok 両方に効く**・resume は内部履歴で代替（CLI の `-r/--continue` は不使用）・auto_detect 除外（`@vibe-kit/grok-cli` 名衝突、`which -a grok` 確認を促す）。背景は SPEC.md § 15.10。
...
- **`cargo install --path .`（`~/.cargo/bin/aish` への配置）はしない**。ユーザ環境の実体は `/usr/local/bin/aish` で、`~/.cargo/bin/aish` と二重になるとデバッグでどちらが動いているか紛れて不便なため（ユーザ指示 2026-09）。ローカルに dev ビルドを入れて検証したい場合は `/usr/local/bin/aish` を上書き（`cargo build --release && sudo install -m 755 target/release/aish /usr/local/bin/aish`）する。
