...
- **Cloudflare Workers AI は native backend `cloudflare`（`src/ai/cloudflare_workers_ai.rs`）。REST を `curl` 経由（`run_cli_capture_stdout`）で叩く＝HTTP クレートを足さない（`--update` と同方針）。`binary()`="curl"**。認証は環境変数のみ（`CLOUDFLARE_ACCOUNT_ID`/`CLOUDFLARE_API_TOKEN`、config 不可）。**呼び出し名は `cloudflare`、設定セクション/ファイル名は `cloudflare-workers-ai` 系**（住み分け。TOML は serde `rename = "cloudflare-workers-ai"`）。curl は HTTP エラーボディを読むため `-f` を付けず `success` を見る。session 無し（gemini/qwen 同型の内部 history）。
