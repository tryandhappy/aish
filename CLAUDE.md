...
- **Windows の自己更新（`--update`）は Unsupported を維持**。ただし**エラーで落とさず、同梱インストーラ `install.ps1` を使う PowerShell one-liner を表示して `Ok(())` で終える**（`run_update` 冒頭で `cfg!(windows)` 分岐 → `windows_update_hint(channel)`。prerelease=`irm … | iex`、stable=`& ([scriptblock]::Create((irm …))) -Stable`）。**Windows インストーラは repo 直下の `install.ps1`（+ ラッパ `install.cmd`）に集約**（raw URL `…/main/install.ps1` で常に最新。arch 判定→最新リリース取得→`Get-FileHash` で sha256 検証→`%LOCALAPPDATA%\Programs\aish` 配置→PATH 追加。既定=`/releases[0]`、`-Stable` で `/releases/latest`）。**手打ちの `$tag` 展開コマンドを README/hint に散らさない**（空変数で HTML が落ちる事故防止）。**既存 `aish.exe` の置換は「old を rename で退避 → 新 exe を Move → old を best-effort 削除」**（`Move-Item -Force` は既存ファイルで `ERROR_ALREADY_EXISTS` を投げ、実行中 exe は上書き不可。ロック中 exe でも rename は可能なのを利用）。Windows バイナリ（x86_64/aarch64 の生 exe + zip）は release に同梱済み（`release.yml` の matrix）。
