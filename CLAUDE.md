...
- **bash の打ちかけ入力消去 (Ctrl+A + Ctrl+K = `0x01 0x0b`) は AI 提案コマンドの「最初の実行」直前 1 回だけ PTY に送る** (`src/main.rs` の AI 確認ループで `!any_executed` のときに送出)。show_minibuffer 終了時 (= AI 質問送信時) には送らない。**ただし shell プロンプトをリフレッシュする改行送信 (slash command 処理後 = `src/main.rs:536` 付近 / AI 対話終了後 = `src/main.rs:754` 付近の `pty.write`) は、その改行の直前に必ず `0x01,0x0b` で行消去してから送ること**。これをしないと、ユーザが Ctrl+/ の前に打ちかけた未確定 shell 入力 (passthrough で既に PTY に届き bash readline に残っている) を、リフレッシュ用の改行が勝手に submit して実行してしまう (= ユーザが Enter していない・承認していないコマンドの実行。信頼の根幹に触れる)。AI 提案コマンドを 1 つでも実行した経路では `!any_executed` の行消去で既に空行なので no-op。打ちかけが本当に温存されるのは **minibuffer を cancel (ESC/Ctrl+C/Ctrl+/) した場合と空 Enter の場合だけ**で、これらは改行を PTY に送らない経路 (PassthroughEnded / 空 prompt) なので readline 上で編集を継続できる。Ctrl+C (`0x03`) ではなく Ctrl+A+Ctrl+K を使うのは SIGINT を発火させずに行消去だけしたいため (vim/top 等の子プロセスを意図せず kill しない)。bash プロンプト直後 (打ちかけなし) で送っても no-op なので害なし。後続の AI 提案コマンドは前のコマンドが完了して bash プロンプトに戻った後送られるので追加で送らない。bash readline 以外 (vim 子プロセス等) に届くと `^A^K` がリテラル文字として流れる副作用は残るが、SIGINT 直撃よりは穏当な失敗モード。
