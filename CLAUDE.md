...
- **Generic backend (`BackendKind::Generic(u8)`) は `[[ai.providers]]` registry 経由で動的解決される**。registry は `BackendKind::init_generics(&providers)` で **OnceLock に 1 度だけ populate** され、`Box::leak` で `&'static str` (display_name / binary) と `&'static ProviderRecipe` (recipe) を確保する。プロセス全期間で生存。`as_str()` / `binary()` の戻り型は `&'static str` のまま維持されるので呼び出し側の変更は不要。init 前 / index 範囲外の Generic にはフォールバック (`"generic:?"` / `"?"`) を返すので panic はしないが、spawn は失敗する。新たに enum variant を増やす場合は `all_native()` / `NATIVE_COUNT` / `ordinal()` の native 分岐を更新する (Generic は自動)。
...
- **`BackendKind::parse("generic:<name>")` は registry init 前に呼ぶと "no [[ai.providers]] configured" エラーになる**。`main::run()` で `config::Config::load` → `BackendKind::init_generics(&config.ai.providers)` → 以降 backend kind parse という順序を守ること。テストでは Generic resolution を直接検証しない (`OnceLock` がプロセス全期間共有なので並列テストで干渉する)。Generic 駆動の動作確認は `src/ai/generic.rs` の単体テストで `Box::leak(Box::new(recipe))` 直書きで行う。
